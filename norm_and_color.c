	...
	t_plane *planes, t_vec3f *norm_dir)
{
	...
	//plane_pos.x = fabsf(hit_point.x);
	//plane_pos.y = fabsf(hit_point.z);
	//*norm_dir = vec3f_init(0, 0, 1);
	//bump_x = ((int)(fabsf(plane_pos.x) / 5 * plane.bump_map.width)) % plane.bump_map.width;
	//bump_y = ((int)(fabsf(plane_pos.y) / 5 * plane.bump_map.height)) % plane.bump_map.height;
	//test_color.x = (float)plane.bump_map.data[(bump_x + bump_y * plane.bump_map.width) * 4] / 255;
	//test_color.y = (float)plane.bump_map.data[(bump_x + bump_y * plane.bump_map.width) * 4 + 1] / 255;
	//test_color.z = (float)plane.bump_map.data[(bump_x + bump_y * plane.bump_map.width) * 4 + 2] / 255;
